...
class RoastBot(commands.Bot):
    ...
    async def generate_roast_for_member(
        ...
        intensity_map = {
            1: "مداعبة فكاهية خفيفة ولطيفة، ضحك ودود وبسيط",
            2: "طقطقة كوميدية مرحة على تصريفاته وحركاته",
            3: "ذبة كوميدية متقنة وذكية بتشبيه ساخر ومضحك جداً يفطس ضحك",
            4: "قصف كوميدي عالي المستوى، تشبيهات قوية ومفارقات تفجر الضحك",
            5: "ذبة أسطورية خارقة وقمة في الإبداع الكوميدي، تشبيه تاريخي عجيب ومفاجئ يفطس السيرفر ضحك"
        }
        ...
        topic_instruction = f"الموضوع المستهدف للذب: [{custom_topic}]" if custom_topic else "التقط أدق حركة أو لعبة أو صمته أو تناقض في حالته واصنع منها تشبيهاً كوميدياً ساخراً يفطس ضحك."

        prompt = f"""
أنت 'مستر ذبات' — خوي الاستراحة الفاصل وأخف دم في الديسكورد السعودي.
مهمتك: صناعة ذبة كوميدية ساخرة جداً تفطس السيرفر والضحية ضحك، مبنية على التشبيهات والمفارقات الكوميدية العجيبة!

الضحية المستهدفة: '{member.display_name}'
اللهجة الإجبارية: [{dialect_instruction}]
مستوى الكوميديا والجرأة: [{intensity_desc}]
...
🎯 الشروط الذهبية للذب الكوميدي الناجح:
1. الهدف الأول والوحيد: (الضحك وخفة الدم والوناسة)! الضحية خويك في السيرفر وتطقطق عليه عشان تضحكون كلكم سوى.
2. 🚫 ممنوع منعاً باتاً أسلوب "النفسية" أو النكد أو العصبية أو أسلوب التهاوش والمشاحنات!
3. 🚫 ممنوع نهائياً استخدام عبارات الإسكات والتسفيل الكئيبة مثل:
   - "خلك ساكت بس" / "درملها وخلك ساكت" / "انثبر"
   - "وضعك مزري" / "يكسر الخاطر" / "وضعك منتهي"
   - "تسوقها" كبداية مكررة لكل كلامك
   - تكرار عبارة "لك نص ساعة" في كل ذبة
4. 💡 سر الذبة الممتازة: استخدم (تشبيه كوميدي سعودي واقعي ساخر ومفاجئ) + (مفارقة مضحكة عن حركته أو لعبته) + (قفلة ذكية ساخرة ومرحة)!
5. سطرين بالكثير، عامية بحتة ومتقنة، بدون أي فصحى، ادخل في الذبة مباشرة بدون أي مقدمات أو ترحيب.
"""
        try:
            resp = await generate_content_ai(contents=prompt)
            roast_text = resp.text.strip()

            # Polish out hostile toxic cliches if model leaks them
            for ban in [
                "خلك ساكت بس!", "خلك ساكت بس", "درملها بس وخلك ساكت!", "درملها بس وخلك ساكت",
                "انثبر بس!", "انثبر وخلك ساكت", "وضعك مزري يا رجال خلك ساكت بس!"
            ]:
                roast_text = roast_text.replace(ban, "").strip()
