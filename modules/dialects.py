...
class DialectSynthesisEngine:
    ...
    def _get_fallback_roast(self, dialect_key: str, member_name: str, topic: str) -> str:
        """توليد ذبة احتياطية أصيلة ومضمونة لكل لهجة عند تعذر استجابة الذكاء الاصطناعي بطابع كوميدي ساخر وخفيف دم."""
        fallbacks = {
            "default": f"يا {member_name} وش جوك بسالفة '{topic}'؟ حركات الترقيع هذي ما تمشي، كأنك مسوي فاهم وأنت مضيع الطاسة ورايح فيها!",
            "riyadh": f"ياخي اركد يا {member_name} بموضوع '{topic}'! مسوي فيها الواثق والمحنك وأنت مضيع الجادة، معطينك جوك بالسيرفر بس!",
            "jeddah": f"يا واد إيش الهرجة دي يا {member_name}؟ دحين مسوي زحمة والشارع فاضي على سالفة '{topic}'؟ روقنا يا كابتن وبلاش دراما!",
            "qassim": f"وش نوحك متوهق تسذا يا {member_name} بسالفة '{topic}'؟ مير اركد يا مال الغنيمة وخل عنك التفلسف اللي ما وراه سنع!"
        }
...
def get_comparative_prompt(topic: str, member_name: str = "الضحية") -> str:
    """
    توليد برومت للمقارنة الفورية بين اللهجات الأربع بطابع كوميدي ساخر ومرح.
    """
    return f"""أنت مستر ذبات، خوي الاستراحة الفاصل وصاحب خفة الدم وسرعة البديهة والقفشات المضحكة.
المطلوب صياغة 4 ذبات كوميدية ساخرة وخفيفة دم لنفس الموضوع ولنفس الشخص ({member_name})، كل ذبة بإحدى اللهجات الأربع بنكهة أصيلة ومضحكة وبدون أي نفسية أو تهاوش.

الموضوع: {topic}

اللهجات المطلوبة:
1. default (عامية شبابية معاصرة ومفهومة: حركات ترقيع، مضيع الطاسة، مسوي فاهم، فاصل، يا رجال)
2. riyadh (لهجة أهل الرياض / نجدية ببرود وسخرية خفيفة دم: اركد، مالك لوا، مروقين، مضيع الجادة، وش تبي)
3. jeddah (لهجة أهل جدة / حجازية سريعة وخفيفة دم: يا واد، إيش الهرجة، بلاش زحمة، يا بويا، روقنا)
4. qassim (لهجة قصيمية طريفة ونصوحة: وش نوحك، مير، تسذا، يا مال الغنيمة، اركد)

{NEGATIVE_CONSTRAINTS}

أجب بصيغة كائن JSON صالح فقط بدون أي وسوم ماركداون أو نصوص خارج الكائن:
{{
